...
class N_QueensProblem(State):
    ...
    def h(self):
        ...
        #   [0, 0, 0, 0] ]
        heuristics = 0
        for n_line in range(self.size):
            for n_collunm in range(self.size):
                if self.board[n_line][n_collunm] == 1 :
                    # Insert queen behavior below;
                    for i in range(self.size):
                        for j in range(self.size):
                            # checa se há outa rainha na mesma linha;
                            if (1 in self.board[n_line][n_collunm,:] or 1 in self.board[n_line][:,n_collunm]):
                                pass
                            # checa se há outra rainha na mesma coluna;
                            if (1 in self.board[n_line,:] or 1 in self.board[:,n_collunm]):
                                pass
